...
if __name__ == "__main__":
    ...
    q = "who expressed mirth"
    import nltk
    lem = nltk.stem.wordnet.WordNetLemmatizer()
    lemma_sent = lem.lemmatize(q)
    ...
    laugh_synsets = wn.synset("laugh.v.01")
    print("\n'Laugh' synsets: %s" % laugh_synsets)

    print("'Laugh' lemmas")
    # for laugh_synset in [laugh_synsets]:
    print(laugh_synsets.lemma_names())
    # look up in dictionary
    print("\n'%s' is in our dictionary: %s" % (laugh_synsets.name(), (laugh_synsets.name() in verb_ids)))